...
search(String slug, GitHub gh, bool v) async {
  ...
  if (v) {
    print('[4/] Fetching $slug\'s JSON');
  }
  if (res.statusCode == 200) {
    var pBody = convert.jsonDecode(res.body);
    if (v) {
      print('[5/] Parsing $slug\'s JSON');
    }
    ...
    for (i; policies.length; i++) {
      ...
      if (v) {
        print('[6/] Fetching $slug\'s policy ($i)');
      }
      if (resp.statusCode == 200) {
        var body = resp.body;
        var px = 0;
          ...
          if (pBody.rubric[px].citations.length >= 1) {
            ...
            for (cx; pBody.rubric[px].citations.length <= cx; cx++) {
              if (v) {
                ...
                print('[7/] Scrubbing $cite for issues');
              }
            ...
            if (scrubCitation(body, pBody.rubric[px].citations[0])) {
              ...
              if (v) {
                var rubricSlug = pBody.rubric[px].slug;
                print('[8/] Generating issue for $slug ($rubricSlug)');
              }
              createIssue(
                  gh, slug, pBody.rubric[px].citation[i], pBody.policies[i]);
            }
